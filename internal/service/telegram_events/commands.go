...
func (p *serv) doTextCmd(ctx context.Context, text string, chatID int, user *tgClient.From) error {
...
func (p *serv) sendHelp(ctx context.Context, chatID int, user *tgClient.From) error {
	return p.tgClient.SendMessage(ctx, chatID, getHelpMsg(user.LanguageCode))
}

func (p *serv) sendHello(ctx context.Context, chatID int, user *tgClient.From) error {
	return p.tgClient.SendMessage(ctx, chatID, getHelloMsg(user.Username, user.LanguageCode))
}

func (p *serv) sendTranscription(ctx context.Context, meta Meta, filePath string, lang *string) (err error) {
